...
class Photoblocks_Public {
	...
	public function shortcode_handler( $atts ) {
		...
		if ( $data == null ) {
			...
			if ( $this->lightbox( $atts['id'] ) == 'fancybox' ) {
				wp_enqueue_script( 'fancybox3', plugin_dir_url( __FILE__ ) . 'premium/jquery.fancybox.min.js', array( 'jquery' ), '3.5.6', false );
				wp_enqueue_style( 'fancybox3', plugin_dir_url( __FILE__ ) . 'premium/jquery.fancybox.min.css', array(), '3.5.6', 'all' );
			}
	...
	public function populatePostBlocks( $gallery, &$blocks ) {
		if ( photob_fs()->can_use_premium_code__premium_only() ) {
			...
			} elseif ( count( $posts ) > count( $blocks ) ) {
			...
			foreach ( $posts as $post ) {
				...
				if ( $found ) {
					...
					if ( is_array( $thumb ) ) {
						...
						$block->image->width  = $meta['width'];
						$block->image->height = $meta['height'];
						$block->image->ratio  = $block->image->width / $block->image->height;
					}
