...
class Photoblocks_Admin {
	...
	public function save_gallery() {
		...
		if ( check_admin_referer( 'photoblocks', 'photoblocks' ) ) {
			...
			if( isset( $_POST['blocks']) ){
				...
				$blocks =  json_decode( wp_unslash( $_POST['blocks'] ), true ); // phpcs:ignore
	...
	public function init() {
		...
		if ( isset( $_GET['action'] ) ) {
			...
			if ( $_GET['action'] == 'delete' && isset( $_GET['id'] ) ) {
				$wpdb->delete( $wpdb->photoblocks, array( 'id' => absint( $_GET['id'] ) ), array( '%d' ) );
				wp_redirect( admin_url( 'admin.php?page=' . $this->plugin_name ), 301 );
				exit();
			}
			if ( $_GET['action'] == 'clone' && isset( $_GET['id'] ) ) {
				$wpdb->get_results(
					$wpdb->prepare( "insert into $wpdb->photoblocks (created, updated, name, data, blocks) select now(), now(), concat(name, ' (cloned)'), data, blocks from $wpdb->photoblocks where id=%d", absint( $_GET['id'] ) )
				);
				wp_redirect( admin_url( 'admin.php?page=' . $this->plugin_name ), 301 );
				exit();
			}
	...
	public function menu() {
		...
		$this->add_page             = add_submenu_page( $this->plugin_name, __( 'Add new', 'photoblocks' ) . ' - PhotoBlocks', __( 'Add new', 'photoblocks' ), 'edit_posts', $this->plugin_name . '-edit', array( $this, 'edit' ) );
		$this->troubleshooting_page = add_submenu_page( $this->plugin_name, __( 'Troubleshooting', 'photoblocks' ) . ' - PhotoBlocks', __( 'Troubleshooting', 'photoblocks' ), 'edit_posts', $this->plugin_name . '-troubleshooting', array( $this, 'troubleshooting' ) );
	}
	...
	public function generate_admin_action_link( $id, $action = 'edit' ) {
		...
		switch ( $action ) {
			case 'delete':
				$link = add_query_arg( array( 'action' => 'delete', ), $link );
				break;

			case 'clone':
				$link = add_query_arg( array( 'action' => 'clone', ), $link );
				break;
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name . '-grid', plugin_dir_url( __FILE__ ) . 'js/photoblocks.grid.js', array( 'jquery' ), $this->version, false );
